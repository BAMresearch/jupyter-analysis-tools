...
import argparse, pandas, itertools, os, sys
import numpy as np
from numpy import zeros, mean, sqrt, std, reshape, size, linspace
from numpy import argsort, ones, array, sort, diff, log10, shape
...
def argparser():
    ...
    parser.add_argument("-q", "--qMin", type = float, default = 0.,
            help = "Minimum Q to clip from original data")
    parser.add_argument("-Q", "--qMax", type = float, default = np.inf,
            help = "Minimum Q to clip from original data")
...
class reBin(object):
    ...
    def __init__(self, **kwargs):
        ...
        for kw in kwargs:
            if self.verbose:
                print('Processing input argument {}: {}'
                        .format(kw, kwargs[kw]))
    ...
    def readFile(self, filename):
        if self.verbose:
            print('reading file: {} with settings: {}'
                    .format(filename, self.pandasArgs))
